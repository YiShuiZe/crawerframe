...
type myRegistrar struct {
	// moduleTypeMap 代表 组件类型 和 对应组件实例 的映射
	moduleTypeMap map[Type]map[MID]Module // 双层的字典结构
	// rwlock 代表组件注册器专用读写锁
	rwlock sync.RWMutex
}
...
func (registrar *myRegistrar) Register(module Module) (bool, error) {
	if module == nil {
		return false, errors.NewIllegalParameterError("nil module istance")
	}
	mid := module.ID()
...
// Get 用于获取一个指定类型的组件的实例
// 本函数会基于负载均衡策略返回实例，返回最“空闲”的那个组件实例
