...
type myScheduler struct {
	// maxDepth 代表爬去的最大深度。首次请求的深度为0
	maxDepth uint32
	// acceptedDomainMap 	代表可以接收的URL的主域名的字典
...
func (sched *myScheduler) download() {
	go func() {
		for {
			...
			if err != nil {
				logger.Warnln("The request buffer bool was closed. Break requet reception.")
				break
			}
