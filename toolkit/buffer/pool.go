...
func (pool *myPool) putData(buf Buffer, datum interface{}, count *uint32, maxCount uint32) (ok bool, err error) {
	...
	if *count >= maxCount && pool.BufferNumber() < pool.MaxBufferNumber() {
		pool.rwlock.Lock() // 双检锁（中的写锁） 1.防止向已关闭的缓冲池追加缓冲器 2.防止缓冲器的数量超过最大值
		...
		pool.rwlock.Unlock()
		*count = 0 // 清零count的值，及时清零“已满”计数可以有效减少不必要的操作和资源消耗
	}
	return
}
...
func (pool *myPool) getData(buf Buffer, count *uint32, maxCount uint32) (datum interface{}, err error) {
	...
	datum, err = buf.Get()
	if datum != nil {
		atomic.AddUint64(&pool.total, ^uint64(0)) // 成功从缓冲器中拿出一条数据后，就将缓冲池所持有的数据总数total减1
		return
	}
	if err != nil {
		return
...
// putData和getData方法中对rwlock的读锁或写锁的锁定就是为了预防关闭bufCh可能引发的运行时恐慌。显然，这些操作能够起作用的前提是Close方法对rwlock的合理使用，
func (pool *myPool) Close() bool {
	...
	close(pool.bufCh) // 关闭pool中的bufCh通道
	for buf := range pool.bufCh {
		buf.Close()
	}
	return true
